...
private const val SHORT_TERMS_HE = """אוטוקבלה מפיקה קבלות ב-iCount עבור תשלומים (ביט, פייבוקס) שאתם משתפים עם האפליקציה.

• הניתוח מתבצע מתוך צילום המסך שתשתפו; פרטי התשלום (סכום, שם, תיאור) נשלחים ל-iCount לצורך הפקת הקבלה בפועל.
...
1. כללי
אוטוקבלה היא אפליקציה המסייעת בזיהוי תשלומים (ביט, פייבוקס ואמצעים דומים) מתוך צילומי מסך שהמשתמש משתף עם האפליקציה, ובהפקת קבלות עבורם באמצעות iCount. השימוש באפליקציה הוא באחריות המשתמש בלבד.
...
4. פרטיות והרשאות
האפליקציה מעבדת רק צילומי מסך של אישורי תשלום שהמשתמש בוחר לשתף איתה באופן פעיל (באמצעות תפריט השיתוף של המכשיר), ואינה קוראת או מעבדת התראות או תוכן אחר באופן אוטומטי. תמונת אישור התשלום שתשתפו נשלחת לשירותי זיהוי טקסט ובינה מלאכותית (כגון Google Vision, Google Gemini, OpenAI ו-Anthropic) לצורך חילוץ פרטי התשלום (סכום, שם משלם, תאריך) מתוך התמונה. לצורך הפקת קבלה בפועל, פרטי התשלום הרלוונטיים נשלחים ל-iCount באמצעות ה-API שלה ונשמרים במערכות iCount בהתאם למדיניות הפרטיות שלה. מדיניות פרטיות מפורטת תפורסם בנפרד ותהיה זמינה בתוך האפליקציה וב-Google Play.
